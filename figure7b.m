%% Figure 7b: Outbreak Type by assortivity and vaccination coverage
eps_range = 0:0.001:1; ppV = 0.85; pImm = 0.3; tspan=[0 500];
tau_range=0:1:500;

contour_val=zeros(length(tau_range),length(eps_range));

for i=1:1:length(tau_range)
...
contourf(tau_range,eps_range,contour_val','LineWidth',2)
map = [
    0.7 0.45 0.88
    0 0.45 0.74 
    0.85 0.33 0.1
...
colormap(map)
xlabel('\tau_b'), xlim([0,500])
ylabel('\epsilon'), ylim([0,1])
